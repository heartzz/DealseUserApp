...
fun getProgressDialog(context: Context, isCancellable: Boolean = false, dialogDismissed: (dialog: DialogInterface) -> Unit = {}): MyCustomProgressDialog? {
    val myCustomProgressDialog = MyCustomProgressDialog(context)
    myCustomProgressDialog.isIndeterminate = true
    myCustomProgressDialog.setCancelable(isCancellable)
    myCustomProgressDialog.setCanceledOnTouchOutside(false)
    myCustomProgressDialog.show()
    myCustomProgressDialog.setOnCancelListener {
        dialogDismissed(it)
    }
    return myCustomProgressDialog
}
