...
class PermissionCheckerActivity : BaseActivity() {
    ...
    private fun checkAndRequestPermissions(): Boolean {
        ...
        PermissionChecker.allPermission.filterTo(listPermissionsNeeded) {
                ...
                when {
                    !perm.contains(false) -> {
                        ...
                        listPermissionsNeeded.indices.forEach { j ->
                            when {
                                Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> perm1 += shouldShowRequestPermissionRationale(
                                    listPermissionsNeeded[j]
                                )
                            }
                        }
                        when {
